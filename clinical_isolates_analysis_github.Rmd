...
Updated in April 2021 with a new Figure 2B

Note: To protect patient data, the early stages of the following analyses have been pre-analyzed, altered, or excluded from this dataset:
...
isolates_tidy$SITE_BROAD <- factor(isolates_tidy$IsolationSite, levels = c("Lung/Trachea","Urine/Catheter", "ENT/Sinus","Skin/Wound", "Blood","Other"), ordered = TRUE)
isolates_tidy_resistance_profiles$SIR <- factor(isolates_tidy_resistance_profiles$SIR, levels = c("S","I","R"), ordered = TRUE)

###################### Namin Consistency ########################
isolates_tidy$Patient_index <- isolates_tidy$PatientIndex
isolates_tidy$rankedDate <- isolates_tidy$RankedDate
isolates_tidy$repeatPatient <- isolates_tidy$RepeatPatient

isolates_tidy <- isolates_tidy %>% 
  select(-Diabetes, -Hemolysis, - Pigment, -Mucoidy,-Metallic,
         -IsolationSite, -PatientIndex, -RankedDate, -RepeatPatient)
...
panel_c <- isolates_tidy %>% 
  mutate(comorbid = case_when(diabetes == 'nodb' ~ 'No Diabetes',
                              diabetes != 'nodb' ~ 'Diabetes',
                              TRUE ~ 'unknown')) %>% 
  mutate(comorbid = factor(comorbid, levels = c('No Diabetes', 'Diabetes'), ordered = TRUE)) %>% 
  group_by(Patient_index, diabetes, comorbid) %>% summarise() %>% 
...
D.	Number of patients with cystic fibrosis or diabetes. CF removed from this script to de-identify data but is available in manuscript.
...
Code for this figure is available below but it had to be commented out/will not run because CF was removed from publicly shareable data to comply with HIPAA
...
## READ FIGURE 3C Data: values (expected, observed, (o-e)/e, p-value, BH adjusted p-value, binary call) to csv to be loaded into github script
figure_3C_dat <- read_csv('data/figure3c_updated_co_resistance_and_pvalues.csv') %>% 
...
figZ3_dat1 <- isolates_tidy %>% select(Isolate, Patient_index, MUCOIDY, METALLIC, PIGMENT, HEMOLYSIS, SITE_BROAD) %>%
  mutate(PIGMENT = factor(PIGMENT ,ordered = FALSE),
         HEMOLYSIS = factor(HEMOLYSIS, ordered = FALSE)) %>%
  arrange(Isolate) %>% ungroup()
...
# Gower Distance
figZ3a_dat_gower <- daisy((figZ3a_dat %>% select(-Isolate, -Patient_index, -SITE_BROAD)), metric = 'gower')
# Classes (with and without separating out CF and Non-CF isolates)
figZ3a_dat_class <- figZ3a_dat %>% select(Isolate, SITE_BROAD)
# PCoA 
...
# Read in figZ3_dat_site to preserve PPI
figZ3_dat_site <- read_csv('data/fig3CDdat.csv')
...
**Figure S3. Antimicrobial prescription histories for three patients.**
...
The following are exported as csv files: (Table S1 and Table S2 data are generated earlier in the script)

- **Table S3.** Complete list of antimicrobials considered in this study arranged by antimicrobial class in descending frequency of prescription. 
- **Table S4.** Complete list of co-isolating pathogens observed in this study.
...
write_csv(SX_table, 'supplement/SupplementalTable3.csv')
...
write_csv(SXX_table, 'supplement/SupplementalTable4.csv')
...
data_S1_p4 <- figure_3C_dat %>% 
  ...
  select(Figure, variable1, variable2, Metric, Value, pvalue, BH_corrected_pvalue, BH_corrected_pvalue_asterisks) %>% 
  arrange(variable1, variable2)
data_S1_p5 <- read_csv('data/figure3_5_a_pvalues.csv') %>% 
...
data_S2 <- isolates_tidy %>% 
  mutate(IsolationSite = SITE_BROAD, PatientIndex = Patient_index, RepeatPatient = repeatPatient, 
         RankedDate = rankedDate, Diabetes = diabetes, Mucoidy = MUCOIDY, Metallic = METALLIC, Hemolysis = HEMOLYSIS, Pigment = PIGMENT) %>% 
  select(Isolate, PatientIndex, RepeatPatient, Season, RankedDate, Age, Sex, Diabetes, IsolationSite, Mucoidy, Metallic,Hemolysis, Pigment, ColorNote) %>% 
