...
### FIGURE 3B ####
# Read in correlation coefficients and p values (for patient privacy)
figure3c_peasrsonCorr2 <- read_csv('data/figure3c_peasrsonCorr2.csv')
figure3c_pvalues_cf <- read_csv('data/figure3c_pvalues_cf.csv')
figure3c_pvalues_nocf <- read_csv('data/figure3c_pvalues_nocf.csv')
# Factor antibiotics:
figure3c_peasrsonCorr2 <- figure3c_peasrsonCorr2 %>% mutate(ABX2 = factor(ABX2, levels = c('Tobramycin', 'Piperacillin/Tazobactam','Meropenem',
                                        'Gentamicin','Ciprofloxacin','Cefepime','Amikacin'), ordered = TRUE))
...
figure3C <- figure3c_peasrsonCorr2 %>% filter(cystic_fibrosis != 'cf') %>% 
...
data_S1_p3 <- figure3c_pvalues_cf %>% inner_join((figure3c_peasrsonCorr2 %>% filter(cystic_fibrosis == 'cf') %>% mutate(ABX2 = as.character(ABX2))), by = c('ABX1','ABX2','cystic_fibrosis')) %>% 
  ...
  select(Figure, variable1, variable2, Metric, Value, pvalue, BH_corrected_pvalue, BH_corrected_pvalue_asterisks) %>% 
  arrange(variable1, variable2)
data_S1_p4 <- figure3c_pvalues_nocf %>% inner_join((figure3c_peasrsonCorr2 %>% filter(cystic_fibrosis != 'cf')%>% mutate(ABX2 = as.character(ABX2))), 
                                                   by = c('ABX1','ABX2','cystic_fibrosis')) %>% 
  filter(!is.na(pearsonCorr)) %>% 
